...
function Coupon({ coupons }, props: Props) {
  const item = (coupon) => {
    ...
    return (
      <CouponItem className="used animate__fadeInUp animate__animated">
        <Row gutter={10}>
          <Col xs={8}>
  ...
  return <>{coupons && coupons.length ? coupons.map((coupon) => item(coupon)) : <BlankText>Танд одоогоор ашигласан купон байхгүй байна.</BlankText>}</>;
}
